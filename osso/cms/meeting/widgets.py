...
__all__ = ('SelectDateWidget', 'SelectTimeWidget', 'SplitSelectDateTimeWidget')

# Attempt to match many time formats:
# Example: "12:34:56 P.M."  matches:
# ('12', '34', ':56', '56', 'P.M.', 'P', '.', 'M', '.')
# ('12', '34', ':56', '56', 'P.M.')
# Note that the colon ":" before seconds is optional, but only if
# seconds are omitted.
...
class SelectTimeWidget(Widget):
    ...
    def render(self, name, value, attrs=None):
        try:  # try to get time values from a datetime.time object (value)
            ...
            if isinstance(value, basestring):
                ...
                if match:
                    ...
                    hour_val = int(time_groups[HOURS]) % 24  # force to 0..24
        ...
        # If we're doing a 12-hr clock, there will be a meridiem value,
        # so make sure the hours get printed correctly.
        if self.twelve_hr and self.meridiem_val:
            if (self.meridiem_val.lower().startswith('p') and hour_val > 12 and
                    hour_val < 24):
        ...
        # For times to get displayed correctly, the values MUST be
        # converted to unicode.  When Select builds a list of options,
