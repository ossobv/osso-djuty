...
# Only test it if it's loaded. It is deprecated after all..
@skipUnless(
    'osso.l10n.middleware.L10nMiddleware' in settings.MIDDLEWARE_CLASSES,
    'Only testing L10nMiddleware if it is used')
class LanguageSelectTest(TestCase):
    ...
    def setUp(self, **kwargs):
        ...
        self.SETTINGS_LANGUAGES = settings.LANGUAGES
    ...
    def test_select_default_language(self):
        ...
        self.assertEqual(
            response._headers.get('content-language', (None, ''))[1], 'da')
    ...
    def test_select_fallback_language(self):
        ...
        response = self.client.get(
            '/', HTTP_ACCEPT_LANGUAGE=self.ACCEPT_LANGUAGE)
        self.assertEqual(
            response._headers.get('content-language', (None, ''))[1], 'nl')
