...
import ast, ssl, unittest, urllib, urllib2, urlparse
...
try: from osso.aboutconfig.utils import aboutconfig
except ImportError: aboutconfig = None


BANKS_MARCH_2012 = ("[{'id': 31, 'name': u'ABN AMRO'}, {'id': 761, 'name': u'ASN Bank'}, "
...
E_BANK_DOWN = -0x4321 # mollie uses negative indexed errors.. we add this custom one


class Ideal(BaseIdeal):
    def __init__(self, testing=False, partner_id=None, profile_key=None, api_url=None):
        mollie_settings = settings and getattr(settings, 'OSSO_PAYMENT_MOLLIE', {}) or {}
        # Store args
        self.testing = bool(testing)
        self.partner_id = partner_id or mollie_settings['partner_id']
        self.profile_key = profile_key or mollie_settings.get('profile_key', None)
        self.api_url = api_url or mollie_settings.get('api_url', 'https://secure.mollie.nl/xml/ideal')
    ...
    def get_banks(self):
        ...
        try:
            ...
            if aboutconfig:
                banks = ast.literal_eval(aboutconfig('payment.provider.mollie.banks', BANKS_MARCH_2012))
            else:
                banks = ast.literal_eval(BANKS_MARCH_2012)
        else:
            banks = banks2dictlist(response)
            if aboutconfig:
                aboutconfig('payment.provider.mollie.banks', repr(banks), set=True)
    ...
    def get_payment_form(self, payment, bank_id):
        ...
        if any(i not in '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz +,-.' for i in description):
            raise BuyerError('Illegal character found in description: %s' % (description,))
        if len(description) > 29:
            raise BuyerError('Size of description too large (max 29): %s' % (description,))

        host = payment.realm.lower()
        if host == 'localhost' or host.startswith('localhost:'):
            host = 'example.com' # Mollie refuses 'localhost', even in test mode
        ...
        if int(order['amount']) != amount:
            raise ProviderError('Payment amount mismatch with request: %s vs. %s (cents)' % (order['amount'], amount), response)
        if len(order['transaction_id']) < 16:
            raise ProviderError('Received transaction_id seems unsafe', order['transaction_id'])
    ...
    def process_report(self, payment, transaction_id):
        ...
        if str(payment.unique_key).rsplit('-', 1)[0] != str(transaction_id):
            raise PaymentSuspect('Found transaction_id %s differing from payment %s' % (transaction_id, payment.id))
        if len(transaction_id) < 16:
            raise PaymentSuspect('Found invalid transaction_id %s in payment %s' % (transaction_id, payment.id))
    ...
    def _do_request(self, params, timeout_seconds=None):
        ...
        if errors:
            ...
            for i in errors:
                ...
                # (-2, u"A fetch was issued without specification of 'partnerid'.")
                # (-2, u'This account does not exist or is suspended.')
                # ...
                if i[0] in (-2, -11, -13, -15, -16):
                    raise ProviderBadConfig('Account does not exist or does not have enough permissions', data)
                # Our custom error from the <order>.
                # (E_BANK_DOWN, 'Your iDEAL-payment has not been setup because of a temporary technical error at the bank')
                if i[0] == E_BANK_DOWN:
                    try:
...
def any2errorlist(xml):
    ...
    for item in dictlist:
        if 'errorcode' in item and item['errorcode'] != '0':
            errors.append((int(item['errorcode']), item.get('message', 'Error message missing')))
    dictlist = dom2dictlist(dom, inside=('response',), li='order', strict=False)
    if len(dictlist) == 1:
        if dictlist[0].get('error', '').strip().lower() == 'true':
            errors.append((E_BANK_DOWN, dictlist[0].get('message', 'Error message missing')))
    return errors
...
class IdealTest(unittest.TestCase):
    ...
    def test_mollie_error_1(self):
        ...
        <response>
            <item type="error">
                ...
                <message>There is a problem with the 'reporturl' you have provided: The URL host must not be 'localhost': localhost</message>
            </item>
            <item type="error">
        ...
        expected = [
            (-3, "There is a problem with the 'reporturl' you have provided: The URL host must not be 'localhost': localhost"),
    ...
    def test_order2dict(self):
        ...
        <response>
            <order>
                <transaction_id>aaabbbcccdddeeefff00011122233344</transaction_id>
                <amount>995</amount>
                <currency>EUR</currency>
                <URL>http://www.mollie.nl/partners/ideal-test-bank?order_nr=M12345&amp;transaction_id=aaabbbcccdddeeefff00011122233344&amp;trxid=0123</URL>
                <message>Your iDEAL-payment has successfully been setup. Your customer should visit the given URL to make the payment</message>
            </order>
        </response>
        '''
        expected = {
            ...
            'amount': '995',
            'currency': 'EUR',
            'URL': 'http://www.mollie.nl/partners/ideal-test-bank?order_nr=M12345&transaction_id=aaabbbcccdddeeefff00011122233344&trxid=0123',
    ...
    def test_url2formdata(self):
        input = 'http://www.mollie.nl/partners/ideal-test-bank?order_nr=M12345&transaction_id=aaabbbcccdddeeefff00011122233344&trxid=0123'
        expected = (
