...
class HttpResponseOk(HttpResponse):
    def __init__(self, message=None, saved=None):
        super(HttpResponseOk, self).__init__(content='[RESPONSE-OK]', content_type='text/plain; charset=UTF-8')


class HttpResponseFail(HttpResponse):
    def __init__(self, message='(unknown)'):
        super(HttpResponseFail, self).__init__(content='[RESPONSE-ERROR: %s]' % message, content_type='text/plain; charset=UTF-8')
...
def delivery_report_forward(request):
    ...
    return simple_form_view(request, form_class=DeliveryReportForwardForm, heading=_('Delivery report forward'),
            ip_whitelist=WIRELESS_IPS, httpresponse_ok=HttpResponseOk, httpresponse_fail=HttpResponseFail, mail_on_fail=True)
